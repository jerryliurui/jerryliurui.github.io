...
最近这段时间，在我和团队一起迭代 `BFS` 这套工作流的时候，我们把 `Graphify` 正式引进到了日常研发流程里，而且不是单独把它当成一个“分析代码的小工具”，而是实打实的塞进了`需求分析`、`方案设计`、`代码实现`、`问题排查`这一整条链路里。
...
前面说的是工具本身，但真正让我觉得这件事有意思的，还不是 `Graphify` 单独存在，而是我和团队把它接进了一套完整的 AI 研发工作流里。

`BFS` 是我和我们团队一起开发的一套面向全栈项目的 AI 研发工具和工作流集合：前面接需求、方案和上下文，后面接任务拆分、实现、归档和知识回流。
