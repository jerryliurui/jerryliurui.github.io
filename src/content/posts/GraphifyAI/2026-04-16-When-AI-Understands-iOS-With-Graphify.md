...
tags: ["iOS", "AI", "Graphify", "Engineering"]
category: iOS
author: JerryLiu
---
...
最近和隔壁一支偏 AI 驱动的研发团队交流时，听他们聊到了一个很有意思的变化：他们把 `Graphify` 引进到了日常研发工作流里，而且不是把它当成一个单独的分析工具，而是把它放进了「需求分析 - 方案设计 - 代码实现 - 问题排查」这一整条链路里。

我听完之后最大的感受是，这件事真正打动人的地方，并不是“又多了一个给 AI 用的工具”，而是它试图去解决一个更本质的问题：**AI 其实一直都不够了解你的工程。**

这一点在 iOS 开发里尤其明显。很多时候 AI 不是不会写代码，而是不知道该从哪一个入口开始改，不知道这个页面背后挂着几个状态源，也不知道一次看似很小的改动到底会不会影响 `Widget`、`Extension`、`App Intents` 甚至别的历史模块。于是我们经常会得到一种“看起来很像对的，但实际接不进工程”的答案。

而 `Graphify` 这类工具的价值，恰恰就在这里：它让 AI 对代码库的理解，从“看见几个文件”往前走了一步，变成“看见这些文件之间是怎么连起来的”。

不过如果只说到这里，这篇文章就还不够完整。因为真正有意思的地方，其实有三层：

1. `Graphify` 到底是什么。
2. 它在实际开发里到底怎么用。
3. 一套 AI 研发工作流是怎么把它接进去的。

下面我就沿着这三个问题来聊。文中涉及到的一些团队信息、模块名、规模数据和目录结构都做了脱敏处理，重点只放在方法和感受本身。
...
过去这一两年，大家其实已经非常习惯让 AI 帮忙写代码了。写一个方法、补一段逻辑、生成一个页面骨架，甚至做一些重构建议，它都能给出看上去还不错的结果。
...
2. 能改当前文件，却漏掉真正相关的调用方和被调用方。
3. 给出一个理论上可行的方案，但和你当前的架构分层完全不匹配。
4. 只看到了页面层，却没意识到这次改动还会影响埋点、路由、缓存、扩展目标和状态同步。

说到底，这些都不是“模型智商不够”的问题，而是**上下文不完整**的问题。

对于一个成熟的 iOS 工程来说，真正重要的信息往往不是某一行 Swift 或 Objective-C 代码本身，而是它所处的位置：它属于哪个模块、被谁调用、依赖哪些协议、和哪个 Target 共用、有没有历史包袱、是不是一条高影响链路上的节点。这些内容，单靠把几个文件扔给 AI，通常是很难补齐的。
...
它做的不是替你写业务代码，而是把原本分散在工程里的结构信息、依赖关系、调用链路、模块边界，用一种更适合机器理解的方式重新组织起来。这样一来，AI 在回答问题、设计方案或者动手改代码之前，就不再只是盯着零散文件，而是能先看到一个相对完整的关系图。

我自己很认同这样一种定位：它既不是人工知识库的替代品，也不是传统代码搜索的替代品，而是它们中间的那一层。
...
这个思路特别像我们自己做技术方案时的习惯。先用经验判断方向，再看工程结构和影响范围，最后回到具体代码里确认细节。只不过以前这套动作主要靠人完成，而现在是把其中很关键的一步，也就是“理解工程关系”这件事，部分前置给 AI 了。
...
也就是说，它既能服务 AI，也能反过来服务人。

### 2.2 对 iOS 工程来说，它关心的是什么

站在 iOS 视角，我觉得它最有价值的不是“把代码扫一遍”，而是它能把下面这些信息提前梳理出来：
...
这类信息平时并不是看不到，而是得靠经验、grep 和反复跳转代码一点点拼出来。`Graphify` 做的事情，就是把这张“拼图”尽量提前拼好。

<a id="how-to-use-graphify"></a>

## 3. Graphify 怎么用

如果只讲定义，还是容易显得太虚。更实际一点的问题是：这东西落到日常开发里，大家到底怎么用？
...
这一步的意义不是“生成报告”，而是让这个模块从此拥有了一份可复用的结构化上下文。后面不管是人看还是 AI 查，都不需要再从零开始扫一遍。

### 3.2 第二步：先查模块全貌，再问局部问题

这一步是我自己最认同的地方。很多时候我们直接把问题扔给 AI，比如“这个功能怎么做”“这个类在哪里改”，其实问题本身就太早了。
...
如果你已经知道要追一条实现链路，那就更适合做深度查询或者路径查询，比如：
...
这时候它的价值就不是“告诉你某个类存在”，而是帮助你理解“这两个点在工程里是怎么连起来的”。

而如果把这一步再往前推一点，其实它对新人和跨端同学也很友好。很多时候，一个 iOS 模块的难点并不是代码本身，而是第一次接触时根本不知道它的边界、核心节点和内部社区是怎么分布的。图谱可视化把这些关系直接摊开之后，即使不是这个模块的长期维护者，也能更快建立起整体认知。

![](./graphify-ios-module-map.png)

上面这类可视化结果，就是一个 iOS 模块跑完图谱之后的典型效果。中间通常是连接更密集的核心区域，四周则是按功能或依赖关系自然聚合出来的若干社区。对于新人来说，它能帮助快速建立“这个模块大概由哪几块组成”的第一印象；对于其他端或者跨模块协作的开发人员来说，它也能明显降低第一次进入陌生 iOS 模块时的理解成本。

### 3.3 第三步：解释某个关键节点

在真实开发里，我们经常会碰到这样一种情况：知道某个类很关键，但不知道它到底关键在哪。
...
为了方便表述，下文我还是沿用对方的叫法，把这套工作流简称成 `BFS`。如果用一句很轻量的话来介绍，它可以理解成是团队内部那套面向全栈项目的 AI 研发工具和工作流集合：前面接需求、方案和上下文，后面接任务拆分、实现、归档和知识回流。这里不展开团队背景，只聊它的方法本身。
...
这里我不想把它讲成一个抽象概念，还是尽量落到 iOS 开发里常见的事情上。
